...
router.post("/manage_users/add", async (req, res) => {
  ...
  try {
    await administratorService.addUsers({
      username,
      password,
      name,
      email,
...
router.get("/manage_articles/detail/:id", (req, res) => {
  ...
  administratorService
    .findByIdArticles(newsID) // Gọi service để lấy thông tin bài viết
    .then((article) => {
...
router.post("/manage_articles/update/:id", (req, res) => {
  ...
  administratorService
    .updatePremium(newsId, isPremium) // Gọi service để cập nhật
    .then(() => {
